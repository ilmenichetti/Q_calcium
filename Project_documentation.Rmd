...
# Introduction
Base cations, and in particular Calcium (Ca), are positively connected with forest fertility but are at the same time highly sensitive to anthropogenic disturbances [@leysNaturalAnthropogenicDrivers2016].
...
The model is driven by littertrap C data and by assumptions about CWD data which are representing the annual inputs for the decomposition model. 
...
When considering instead future climate, we can simulate the effect of ### 2.2.1 Future Ca vulnerability assuming constant input and changing climate
the foreseen change in precipitation in the Ca content of the biomass:
...
This suggests that, as a consequence of natural stresses such as massive plant deaths caused by storms/outbreaks, the system is losing Ca. This seems confirmed by the Cacontent in the stream water from the catchment around a decade after the insect outbreaks following the Gudrun storm.
![Simulation vs measured bulk Ca flux](Figures/Simulation_vs_measured.png)
The lag we observe is probably due to the soil buffer capacity, but ultimately if the biomass does not grow back these events will result in a Ca loss from the syste,
